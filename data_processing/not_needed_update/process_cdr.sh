...
for d in "Test";
do
    python3 process.py --input_file ${input_path}/CDR_${d}Set.PubTator.txt \  #input is the original dataset (PubTator.txt)
                       --output_file ${input_path}/${d} \   # output is : test.data file 
                       --data CDR

    # python3 filter_hypernyms.py --mesh_file 2017MeshTree.txt \
    #                             --input_file ${input_path}/${d}.data \   # input : test.data file 
    #                             --output_file ${input_path}/${d}_filter.data   # output is : test_filter.data
done
